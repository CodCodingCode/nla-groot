...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="W&B project name. When set, the SFT loop calls wandb.init() "
             ...
             "Reads WANDB_API_KEY from env (.env auto-loaded if present). "
             "Default None = no wandb (legacy behavior, byte-identical).",
    )
    p.add_argument(
        "--wandb-run-name", default=None,
        help="Override the autogenerated W&B run name. Default = basename "
             "of --output-dir. Ignored when --wandb-project is unset.",
    )
