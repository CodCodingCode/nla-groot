% nla-groot — CoRL 2026 submission (8 pages main + refs/appendix).
% Build:  make -C paper corl
% Before submit: scrub repo URLs and paths from the PDF.
...
All experiments use a single VLA (GR00T-N1.7-LIBERO), one hook site (layer~16,
pre-action-head LayerNorm), and LIBERO simulation.
We do not establish whether failures persist across layers, action heads,
real robots, or other VLAs~\citep{brohan2023rt2,kim2024openvla}.

\paragraph{Teacher supervision confound.}
Gold captions are produced by a multimodal teacher that sees pixels and
metadata, not \(\hvec\) itself.
Our negative result therefore bounds \emph{teacher-supervised} NLAs under this
protocol; it does not rule out alternative supervision (human \(\hvec\)
descriptions, contrastive pixel alignment, or sparse-autoencoder features).

\paragraph{Evaluation coverage.}
Axis~3 steering is tested on one LIBERO Goal task with three seeds;
\(\Delta_{cw}=0\) may reflect dose miscalibration as well as semantic failure.
Judge subsamples are small for per-\code{position\_type} slices
(e.g., \(n{=}8\) per type targeted).
Counterfactual auditing (Axis~4) is documented but not fully reported here.

\paragraph{Mitigations left open.}
We document but do not fully evaluate label QA, quality-weighted SFT, mined
hard negatives, or judge-blended GRPO.
Future work should test whether these interventions recover
\code{image\_patch} grounding and positive \(\Delta_{cw}\).
