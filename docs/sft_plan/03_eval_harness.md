...
> **Post–V2 (droid_100ep):** High FVE — especially **teacher-forced** — can hide **shorthand templates** that fail **scene grounding**. Run **`scripts/eval/llm_judge_av_captions.py`** (axes **B/C**) on serious checkpoints; enable **`--eval-closed-loop`** on SFT. Rationale: **`docs/evals/v2_lessons_learned.md`**.
